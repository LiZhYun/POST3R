...
def build(config: Dict[str, Any]) -> torchmetrics.Metric:
    """
    Build a metric from configuration (SlotContrast-style).
    
    Args:
        config: Dictionary with metric configuration
                Must contain 'name' key with metric class name
                Other keys are passed as kwargs to metric constructor
    ...
    if 'name' not in config:
        raise ValueError(f"Metric config must contain 'name' key. Got: {config.keys()}")
    
    metric_name = config['name']
    metric_kwargs = {k: v for k, v in config.items() if k != 'name'}
    
    # Try to get metric class from current module (globals)
    if metric_name in globals():
        metric_cls = globals()[metric_name]
        return metric_cls(**metric_kwargs)
    
    # Try to get from torchmetrics
    ...
    raise ValueError(
        ...
        f"Check if metric class exists in post3r.training.metrics or torchmetrics"
    )
